...
export default function AgendaWeekView({
  ...
  const allDayByDay = useMemo(() => weekDays.map((day) => {
    ...
    return dayEv.filter((ev) => eventBelongsInAllDayLane(ev));
  }), [weekDays, events]);
  ...
  const showAllDayLane = dndEnabled || allDayByDay.some((dayEvents) => dayEvents.length > 0);
              ...
              >
                ...
                <Box
                  sx={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    bgcolor: today ? 'primary.main' : (active ? alpha(theme.palette.primary.main, 0.12) : 'transparent'),
                ...
                >
                  <Typography
                    variant="subtitle1"
                    sx={{
                      fontWeight: today || active ? 700 : 500,
                      fontSize: '1.05rem',
                      lineHeight: 1,
                    }}
                  >
